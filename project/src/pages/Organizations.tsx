...
const Organizations: React.FC = () => {
  ...
  const getUnassignedUsers = () => {
    return users.filter(user => !user.organization_id && user.role !== 'customer');
  };
              ...
              <div key={org.id} className="card hover:shadow-lg transition-shadow">
                ...
                <div className="space-y-2 mb-4">
                  ...
                  <div className="flex items-center text-sm text-gray-500">
                    ...
                    <span>Est. {new Date(org.established_date).toLocaleDateString()}</span>
                  </div>
                </div>
                      ...
                      {members.slice(0, 3).map((member) => (
                        <div key={member.id} className="flex items-center justify-between text-sm">
                          ...
                          <span className={`status-badge text-xs ${
                            member.role === 'doctor' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                          }`}>
                            {member.role}
                          </span>
                        </div>
